...
Although we develop the trading-timing instance in full, the construction is a general principle. Whenever a decision sequence is overlaid on an exogenous process, holding the structural footprint fixed and randomizing only the decision margin isolates the decisions' marginal contribution; entry placement on a price path is the leading instance, and Section~\ref{sec:theory} and the discussion sketch the others. The empirical work segments GC=F into Calm, Neutral, and Stressed regimes, since many strategies claim conditional rather than universal edge. Every reported value---returns, trade counts, Monte Carlo diagnostics, regime ratios, and classifications---is reproduced from the canonical trade logs and the structure-preserving null by the accompanying code, which verifies its regenerated statistics against the reported figures and aborts on any mismatch; the simulation-validation results are likewise produced from code in worlds where the truth is known.
...
\paragraph{Local power and relative efficiency.} The detection boundary above is sharp under contiguous alternatives. Consider the Pitman sequence $\zeta_{N_s}=h/\sqrt{N_s}$ for fixed $h\ge0$, under which the realized placement carries a vanishing per-trade edge that scales exactly at the detection rate. Under the same no-dominant-term central-limit regularity, $Z_{N_s}\to_d N(h,1)$ along this sequence, so the level-$\alpha$ one-sided test has asymptotic power
\[
\beta(h)=\Phi\big(h-z_{1-\alpha}\big),
\]
where $z_{1-\alpha}$ is the standard normal quantile: $\beta(0)=\alpha$, $\beta(z_{1-\alpha})=\tfrac12$, and $\beta=0.80$ at $h=z_{1-\alpha}+z_{0.80}\approx2.49$. The test thus attains the local power of the one-sided Gaussian test against the entry-placement location direction and detects edges shrinking no faster than $N_s^{-1/2}$.

The comparison with competing procedures is best stated in terms of \emph{direction} rather than efficiency. Against the same per-trade location alternative, the single-strategy specialization of White's Reality Check \cite{white2000} and Hansen's SPA \cite{hansen2005}---a studentized bootstrap test of mean trade return---shares the noncentrality $h$, so the asymptotic relative efficiency against entry-placement alternatives is one. The procedures diverge against a different direction: under a pure structural-exposure (drift) alternative, in which the strategy is profitable but its placement carries no edge, the structure-preserving test has noncentrality \emph{zero}---randomized placement reproduces the realized exposure, so the realized statistic stays central under $\mathbb{Q}$---whereas the return-based tests have strictly positive noncentrality and reject. The structure-preserving null is, by construction, orthogonal to the exposure direction that drives the competitors' rejections; the empirical pattern of Section~\ref{sec:competitor}, where Reality Check and SPA reject a profitable-but-untimed strategy and the timing test does not, is the finite-sample shadow of this asymptotic orthogonality.
...
\subsection{What this null identifies, and how it differs from return-based tests}

The structure-preserving null isolates incremental entry-placement performance conditional on the realized trade geometry. The logic is forced by the question: to test whether entry placement carries information beyond a strategy's structural burden, the null must vary placement while holding fixed everything being conditioned on; any null that also varies trade count, holding periods, or the return series answers a broader question and reconfounds placement with those sources. Preserving the exact duration and gap multisets---not their means---matters because holding-period structure interacts with the return process (a $5$-bar hold faces a different return distribution than a $50$-bar hold on the same path), and preserving gaps keeps every simulated schedule feasible. Crucially, the null does not resample returns: because all simulated paths are priced on the one realized series, it inherits the market's autocorrelation, fat tails, volatility clustering, and regime dynamics without a parametric return model.

This is what separates it from the standard alternatives. A return bootstrap \cite{politisromano1994,Kunsch1989} resamples returns away from the realized path and so tests profitability under a generic return process, not timing on the path that occurred; return shuffling additionally destroys the dependence that momentum and mean-reversion rules exploit. Both answer a different question, and the cost is concrete rather than rhetorical: Section~\ref{sec:competitor} shows on known-truth data that return-based profit tests (Reality Check, SPA) reject a profitable-but-untimed strategy roughly two-thirds of the time, while the structure-preserving null holds its nominal size. The price of the structure-preserving design is computational---Monte Carlo rather than a closed form---which is favorable when the goal is timing inference with minimal distributional assumptions.
...
The validity argument so far is theoretical: because randomized placement reproduces the realized structural exposure, the structure-preserving null is orthogonal to the exposure that drives profit but not timing (Section~\ref{sec:theory}). We now make that argument visible by running the standard return-based competitors on the same controlled worlds, where the ground truth is known. Four tests are compared on identical data: the structure-preserving timing null of this paper (\textbf{SP}); a moving-block bootstrap of the return path that replays the realized schedule (\textbf{Path}, block length $20$, the path-resampling competitor); the single-strategy specialization of White's Reality Check \cite{white2000} (\textbf{RC}), a recentered stationary-bootstrap test of mean trade return against a zero benchmark; and Hansen's Superior Predictive Ability test \cite{hansen2005} (\textbf{SPA}), its studentized variant. SP and Path ask whether \emph{placement} is informative; RC and SPA ask whether \emph{the mean trade return} is positive. Five worlds span the relevant cases: three no-skill worlds (i.i.d., a pure upward drift, and volatility clustering), the decisive \emph{structural-exposure} world (a genuinely profitable strategy built from drift times exposure but with entries placed at random, so it has \emph{no} timing skill), and a genuine entry-skill world. A correct test of timing should reject only in the last.
...
Figure~\ref{fig:competitor} shows the pattern. The structural-exposure cluster is the crux: it is exactly the configuration the framework was built to handle, and it is exactly where the return-based tests fail. This is the finite-sample shadow of the asymptotic orthogonality of Section~\ref{sec:theory}---the structure-preserving null carries zero noncentrality against the exposure direction, so a profitable-but-untimed strategy stays central under $\mathbb{Q}$, while the profit tests inherit the full exposure signal and reject. The methods do not disagree because one is more powerful; they disagree because they are answering different questions, and only one of them is the timing question. Where a reviewer previously had to trust the orthogonality argument, the controlled experiment makes it something to see.
...
\item trade counts and per-strategy null distributions from the consolidated Monte Carlo panel (Figure~\ref{fig:mc_panel}) and Table~\ref{tab:mc},
...
The central empirical finding is the systematic gap between profitability and timing evidence. Of the ten strategies with positive cumulative returns, only four show even weak evidence of favorable entry placement; the other six are profitable yet statistically indistinguishable from random timing under the structure-preserving null. The reason is direct: gold appreciated over the sample (buy-and-hold return $14.670$), so any sufficiently long-biased rule captures part of that trend regardless of when it enters, and the structure-preserving null absorbs the drift by repricing the same trades on the same appreciating path. This is precisely the overfitting trap the framework is built to catch: a researcher ranking candidates by cumulative return preferentially selects strategies whose \emph{structure}---holding period, frequency, direction---interacts well with the realized path, and reads the resulting exposure as edge.

The embedded random baseline makes the point sharpest. The Random strategy enters with probability $0.07$ per bar, ignores every market feature, and is nonetheless profitable at $0.601$---more than nine of the ten rule-based strategies---yet it classifies as \emph{below null median} ($p=0.710$, percentile $29.0$, $\mathrm{RCSI}$ robustness $-0.103\pm0.523$). Its return comes entirely from long exposure to an appreciating asset; its random timing sits near the center of its own null, and the seed-$42$ realization happens to fall just below. Profit without timing information is exactly what the null is built to expose, and here it does.

\subsection{Anatomy of weak skill}

The four weak-skill strategies---Squeeze Breakout ($p=0.053$, percentile $94.7$), ADX Trend ($p=0.141$, percentile $85.9$), Connors RSI2 ($p=0.143$, percentile $85.7$), and Breakout Vol+Mom ($p=0.176$, percentile $82.4$)---share one trait: each embeds an explicit regime or volatility filter (volatility compression, confirmed trend strength, short-term oversold extremes, or participation-plus-momentum confirmation) that concentrates entries in structurally favorable environments and so acts as an implicit timing mechanism. None crosses $p\le0.05$, but each modestly beats random placement, and their percentile placements are stable across robustness seeds (standard deviations of $0.3$ to $0.5$ points), so the classifications are not artifacts of a single simulation draw. The strategies classified as random or luck, by contrast, use broader entry conditions---Turn-of-Month a calendar rule, Donchian Reentry a breakout-only filter---that dilute any conditional advantage. Specificity in entry conditions, not complexity, is the mechanism through which weak timing emerges.
...
The finding has a direct economic reading. Across a broad panel, entry-timing alpha is largely not separately identifiable from structural exposure to a drifting asset: the same realized return is, to first order, reproducible by randomized placement of the same trades. Three implications follow. For \emph{performance attribution and manager due diligence}, a profitable track record is on its own uninformative about timing skill, so an allocator should require a timing claim to clear a structure-matched counterfactual before paying for it; the test supplies exactly that instrument, model-free and computable from a trade log. For the \emph{market-efficiency debate}, the result sharpens the object of study---rather than asking whether a strategy beats a benchmark, it isolates the single margin (placement) most often mistaken for skill, and finds it scarce and measure-fragile even where profitability is common. And for \emph{strategy development}, the diagnostic redirects effort: a rule whose edge is conditioned away as structural exposure will not survive live trading on a different path, whereas the regimes in which weak placement evidence does concentrate (Section~\ref{sec:realistic-signals} and the regime analysis) mark where genuine, if modest, signal may reside.

\subsection{Beyond entry timing: a general principle}

The construction is not specific to entry timing, to long-only trading, or to gold. It applies whenever a decision sequence is overlaid on an exogenous process and one wishes to isolate the marginal contribution of the decisions from the structural exposure they create. Replacing ``entry placement on a price path'' with the analogous decision--structure pair yields a test for: \emph{factor-timing and tactical asset-allocation} rules (randomize the rebalancing schedule while holding portfolio weights and the realized factor path fixed); \emph{execution algorithms} (randomize child-order placement while holding the parent order and the realized microstructure path fixed); the \emph{evaluation of reinforcement-learning or agentic policies} (randomize action timing while holding the policy's structural footprint fixed, scoring against a structure-matched random policy); and the \emph{timing of macro forecasts or event signals} (randomize the announcement-relative placement). In each case the same three ingredients recur---a fixed exogenous path, a preserved structural profile, and a randomized decision margin---so the finite-sample validity (Proposition~\ref{prop:q-validity}), the local-power and orthogonality results (Section~\ref{sec:theory}), and the calibration machinery transfer directly. We develop the trading-timing instance in full and leave these as the program the framework opens.

\section{Limitations}

Several limitations bound the claims. \textbf{Power and coverage.} Trade counts vary widely---Squeeze Breakout contributes only four trades---so low-frequency strategies are weakly identified (Section~\ref{sec:theory}), and the strongest claims remain daily and long-only; the $14$-instrument walk-forward and the bounded frequency check are exploratory single-run breadth rather than seed-robust inference, and short and mixed-direction strategies, to which the null generalizes, are not evaluated here. \textbf{Between-strategy search.} The test is \emph{within}-strategy: it does not by itself correct for the choice of the eleven rules. We report Benjamini--Hochberg-adjusted $p$-values, compare against the single-strategy Reality Check and SPA in Section~\ref{sec:competitor}, and control multiplicity across the $322$-test panel, but the deeper protection requires pre-registration or out-of-sample replication on assets and windows not used during rule design. \textbf{Measure dependence.} As Section~\ref{sec:theory} formalizes, the verdict is defined relative to a conditioning measure; we report measure invariance as the strongest design-supported claim and cannot rule out genuine state-conditional skill that a measure absorbs. The framework applies mechanically to other assets, frequencies, and directions, but its conclusions scale only with the breadth, power, and out-of-sample discipline of the evidence.
...
\textbf{Code and data availability.} All code and the derived data required to reproduce every table and figure in this paper are publicly available at \url{https://github.com/ampatel355/FORTUNAFRAMEWORK} and permanently archived at \url{https://doi.org/10.5281/zenodo.20695539}. The repository contains the structure-preserving null implementation, the analysis scripts referenced in this appendix, the regime-labeled price series and realized trade logs, and a \texttt{README} mapping each table and figure to its exact reproduction command. Runs are deterministic under the seed protocol described below.
