...
def mc_panel(rows):
    """All eleven null distributions as one small-multiples grid (replaces 11 figures)."""
    n = len(rows)
    ncol = 3
    nrow = int(np.ceil(n / ncol))
    fig, axes = plt.subplots(nrow, ncol, figsize=(11.0, 2.5 * nrow))
    axes = np.atleast_1d(axes).ravel()
    for ax, r in zip(axes, rows):
        s = r["sim"]
        ax.hist(s, bins=42, color=GREY, alpha=0.8, edgecolor="white", linewidth=0.2)
        ax.axvspan(r["p5"], r["p95"], color=BLUE, alpha=0.07)
        ax.axvline(r["median"], color="#555555", ls=":", lw=1.0)
        col = GREEN if r["actual"] >= r["median"] else RED
        ax.axvline(r["actual"], color=col, lw=2.0)
        ax.set_title(f"{r['display']} ({r['n']} tr.)\n$p={r['p']:.3f}$, pct {r['pct']:.0f}",
                     fontsize=8.5)
        ax.tick_params(labelsize=7)
        ax.set_yticks([])
    for ax in axes[n:]:
        ax.axis("off")
    fig.supxlabel("cumulative return (realized line vs. structure-preserving null; "
                  "green above / red below null median)", fontsize=9)
    fig.tight_layout()
    fig.savefig(C / "mc_panel.png", dpi=150); plt.close(fig)
...
def main():
    ...
    mc_histograms(rows); mc_panel(rows); bars(rows); robustness(); regime_heatmap(); equity_curve(market_df)
    print(f"wrote {len(rows)} MC histograms + combined panel + bars + robustness + heatmap + equity curve to Charts/")
